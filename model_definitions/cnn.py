...
class CNN():
    ...
    def __init__(self, num_features, num_historical_days, is_train=True):
        ...
        with tf.compat.v1.variable_scope("cnn"):
            ...
            conv = tf.nn.conv2d(X,filters=k1,strides=[1, 1, 1, 1],padding='SAME')
            ...
            conv = tf.nn.conv2d(relu, filters=k2,strides=[1, 1, 1, 1],padding='SAME')
            ...
            conv = tf.nn.conv2d(relu, filters=k3, strides=[1, 1, 1, 1], padding='VALID')
            ...
            self.confusion_matrix = tf.math.confusion_matrix(tf.argmax(self.Y, 1), tf.argmax(logits, 1))
            tf.compat.v1.summary.scalar('accuracy', self.accuracy)
            theta_D = [k1, b1, k2, b2, k3, b3, W1, b4, W2]           
            
            # D_prob = tf.nn.sigmoid(D_logit)

        self.loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(self.Y), logits=logits))
