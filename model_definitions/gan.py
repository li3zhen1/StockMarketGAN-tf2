...
class GAN():
    ...
    def __init__(self, num_features, num_historical_days, generator_input_size=200):
        ...
        with tf.compat.v1.variable_scope("discriminator"):
            ...
            k2 = tf.Variable(tf.random.truncated_normal([5, 1, 32, 64],
                stddev=0.1, dtype=tf.float32))
            b2 = tf.Variable(tf.zeros([64], dtype=tf.float32))


            k3 = tf.Variable(tf.random.truncated_normal([5, 1, 64, 128],
                stddev=0.1, dtype=tf.float32))
            b3 = tf.Variable(tf.zeros([128], dtype=tf.float32))


            W1 = tf.Variable(tf.random.truncated_normal([3*1*128, 128]))
        ...
        self.D_loss = -tf.reduce_mean(tf.math.log(D_real) + tf.math.log(1.0 - D_fake))
        self.G_loss = -tf.reduce_mean(tf.math.log(D_fake)) 


        self.D_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(self.D_loss, var_list=theta_D)
        self.G_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=0.001).minimize(self.G_loss, var_list=theta_G)
