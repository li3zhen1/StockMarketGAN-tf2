...
class GAN():
    ...
    def __init__(self, num_features, num_historical_days, generator_input_size=20):
        ...
        with tf.compat.v1.variable_scope("generator"):
            ...
            W2 = tf.Variable(tf.random.truncated_normal([128, 512]))
            b2 = tf.Variable(tf.random.truncated_normal([512]))

            h2 = tf.nn.sigmoid(tf.matmul(h1, W2) + b2)


            W3 = tf.Variable(tf.random.truncated_normal([512, generator_output_size]))
        ...
        with tf.compat.v1.variable_scope("encoder"):
            e_h1_size = 256
            e_W1 = tf.Variable(tf.random.truncated_normal([num_features*num_historical_days, e_h1_size]))
        ...
        self.G_solver = tf.compat.v1.train.AdamOptimizer(learning_rate=0.0001).minimize(self.G_loss, var_list=theta_G + theta_E)
